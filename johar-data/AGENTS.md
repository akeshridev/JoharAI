...
## Scope
Android data/runtime module. Depends on `johar-domain` and owns the prebuilt knowledge database, offline retrieval, source discovery, crawling, parsing/extraction, refresh, Room persistence, WorkManager scheduling, Ranchi spatial lookup, and Ranchi offline routing data/runtime.
...
## Ranchi offline routing
- `RanchiOfflineRouter` owns runtime car routing over a prebuilt Ranchi road graph. It performs nearest-road snapping plus A* and returns real route points, road distance, and estimated duration. It does not provide turn-by-turn instructions or live traffic.
- The developer/build-time source artifact is generated by `tools/maps/build-ranchi-routing-pack.sh` from OpenStreetMap data and is written to `app/src/main/assets/routing/ranchi-routing-v1.tsv.gz`.
- Android packaging can expose that gzip source asset inside the APK as `assets/routing/ranchi-routing-v1.tsv` after transparent decompression. Runtime code must therefore use the packaged `.tsv` name as the primary asset contract, while retaining `.tsv.gz` fallback compatibility.
- The loader sniffs gzip magic bytes rather than assuming compression from the asset filename, so both packaged plain TSV and legacy gzip streams are accepted.
- Before debugging route-query parsing, verify the APK directly with `unzip -l app/build/outputs/apk/debug/app-debug.apk | grep assets/routing`. If the packaged route graph is absent, that is an asset/build problem; if it is present but routing reports unavailable, inspect the runtime asset contract.
- Never manufacture route distance, duration, or geometry when the graph is missing or routing fails.
...
- `routing/` — offline Ranchi routing graph loader and A* runtime.
- `spatial/` — Ranchi-scoped coordinate/place lookup and straight-line distance calculations.
...
- Keep seed creation, booster enrichment, retrieval, source fetching, extraction, storage, discovery, routing, spatial lookup, and scheduling independently replaceable.
