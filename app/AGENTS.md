...
- Live/changeable wording such as open now, today, availability, weather, traffic, stock, current/live status, crowded/working state, or schedules must render as `NOT_CONFIRMED` unless a live source is actually wired. Evaluate this guard before route/comparison/itinerary parsing so a current/live query cannot accidentally become an offline factual answer.
- Place matching is exact-first. After exact matching, bounded edit-distance typo tolerance may be used only against real candidates already returned by the spatial resolver. Generic place-type variants such as station/mandir/falls/dam/ground may be normalized. Never invent a place or use fuzzy matching as permission to pick an unrelated candidate.
